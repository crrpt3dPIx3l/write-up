...
## Solution
Started by downloading the Java code provided in the question and analyze the code
```
...
```
The very first step is to make the code readable
```
// These pesky special agents keep reverse engineering our source code and then
// breaking into our secret vaults. THIS will teach those sneaky sneaks a
// lesson.
...
```
To get the flag the bytes have to get into its normal from before scrambling, so I used chatgpt to help me in this and this is the code to unscramble the letter
```
import java.util.*;
...
```
└─$ java java8flag      
s0m3_m0r3_b1t_sh1fTiNg_bbb568cb7
```